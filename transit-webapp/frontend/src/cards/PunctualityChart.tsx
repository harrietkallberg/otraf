...
const PunctualityFlowChart: React.FC<PunctualityFlowChartProps> = ({ 
  ...
  if (!globalData || !direction.canonical_patterns) {
  ...
  const sortedStops = Object.entries(direction.canonical_patterns)
          ...
          <div className="bg-gray-50 rounded-lg p-3">
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                ...
                <div className="text-lg font-bold" style={{ color: getColor(maxValue, selectedMetric).replace(/rgba?\(([^)]+)\)/, 'rgb($1)').replace(/, [\d.]+\)/, ')') }}>
              ...
              <div>
                ...
                {(() => {
                  const dataWithValues = performanceData.filter(d => d.hasData)
                  let bestStop
                  
                  if (selectedMetric === 'on_time') {
                    // For on_time, highest is best
                    bestStop = dataWithValues.reduce((max, stop) => stop.value > max.value ? stop : max)
                  } else {
                    // For too_late and too_early, lowest is best
                    bestStop = dataWithValues.reduce((min, stop) => stop.value < min.value ? stop : min)
                  }
                  
                  return (
                    <>
                      <div className="text-lg font-bold text-green-600">
                        {bestStop.value.toFixed(1)}%
                      </div>
                      <div className="text-xs text-gray-500 mt-1">
                        at {bestStop.stopName}
                      </div>
                    </>
                  )
                })()}
              ...
              <div>
                ...
                {(() => {
                  ...
                  if (selectedMetric === 'on_time') {
                    // For on_time, lowest is worst
                    worstStop = dataWithValues.reduce((min, stop) => stop.value < min.value ? stop : min)
                  } else {
                    // For too_late and too_early, highest is worst
                    worstStop = dataWithValues.reduce((max, stop) => stop.value > max.value ? stop : max)
                  }
                  
                  return (
                    <>
                      <div className="text-lg font-bold text-red-600">
                        {worstStop.value.toFixed(1)}%
                      </div>
                      <div className="text-xs text-gray-500 mt-1">
                        at {worstStop.stopName}
                      </div>
                    </>
                  )
                })()}
