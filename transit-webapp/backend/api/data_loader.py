...
def load_route_file(route_id, filename):
    ...
    full_path = f'route_{route_id}/{filename}'
    return load_data_file(full_path)

def list_user_files():
    """List all files for the authenticated user."""
    user_id, auth_token, refresh_token = get_user_id_and_tokens()  # Get user ID, auth_token, and refresh_token
    return list_files_in_user_bucket(user_id, auth_token, refresh_token)  # Call the function in supa_helper.py to list files
